...
class Evaluator:
    ...
    def evaluate_classification(
        ...
        has_suggestions = (
            "suggestions" in predicted and len(predicted["suggestions"]) > 0
        )

        suggestion_confidence_avg = 0.0
        if has_suggestions:
            confidences = [s.get("confidence", 0.0) for s in predicted["suggestions"]]
    ...
    def generate_report(
        ...
        for error_type, metrics_list in metrics_by_error_type.items():
            stage_acc = (
                sum(1 for m in metrics_list if m.correct_stage) / len(metrics_list)
                if metrics_list
                else 0
            )
            sev_acc = (
                sum(1 for m in metrics_list if m.correct_severity) / len(metrics_list)
                if metrics_list
                else 0
            )
            complex_acc = (
                sum(1 for m in metrics_list if m.correct_complexity) / len(metrics_list)
                if metrics_list
                else 0
            )
            avg_time = (
                sum(m.response_time_seconds for m in metrics_list) / len(metrics_list)
                if metrics_list
                else 0
            )
            avg_conf = (
                sum(m.suggestion_confidence_avg for m in metrics_list)
                / len(metrics_list)
                if metrics_list
                else 0
            )
            has_sugg = (
                sum(1 for m in metrics_list if m.has_suggestions) / len(metrics_list)
                if metrics_list
                else 0
            )
        ...
        return PerformanceReport(
            ...
            avg_stage_accuracy=round(
                (stage_correct / total * 100) if total > 0 else 0, 2
            ),
            avg_severity_accuracy=round(
                (severity_correct / total * 100) if total > 0 else 0, 2
            ),
            avg_complexity_accuracy=round(
                (complexity_correct / total * 100) if total > 0 else 0, 2
            ),
            avg_suggestion_confidence=round(
    ...
    def print_report(report: PerformanceReport) -> None:
        ...
        print(
            f"  Complexity Classification Accuracy:  {report.avg_complexity_accuracy}%"
        )
        print(
            f"  Average Suggestion Confidence:       {report.avg_suggestion_confidence}"
        )
        ...
        for error_type, metrics in report.results_by_error_type.items():
            ...
            print(
                f"    - Complexity Accuracy:         {metrics['complexity_accuracy']}%"
            )
            print(
                f"    - Suggestion Confidence:       {metrics['avg_suggestion_confidence']}"
            )
            print(
                f"    - Suggestions Generated:       {metrics['suggestions_generated_rate']}%"
            )
            print(
