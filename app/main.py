...
async def health_check() -> Dict[str, str]:
    ...
    return {"status": "healthy", "version": "0.1.0", "service": "AI PLC Diagnostic API"}
...
async def classify_error(payload: Dict[str, str]) -> DiagnosticReport:
    ...
    if not log_text or not xml_content:
        ...
        raise HTTPException(
            status_code=400, detail="Payload must include 'log_text' and 'xml_content'."
        )
    ...
    logger.debug(
        f"Parser identification: Stage={metadata['stage']}, Line={metadata['line']}"
    )
    ...
    logger.success(
        f"Classification success: {metadata['stage']}. Metadata & Fixes ready."
    )
