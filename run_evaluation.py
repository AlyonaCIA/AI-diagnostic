...
def run_evaluation(num_constant_errors: int = 10, num_code_gen_errors: int = 10):
    ...
    all_errors = ErrorGenerator.generate_all_test_cases(
        constant_error_count=num_constant_errors, code_gen_count=num_code_gen_errors
    )
    ...
    for i, error in enumerate(all_errors, 1):
        try:
            ...
            status = (
                "✓"
                if (
                    diagnostic_report.stage == error.expected_stage
                    and diagnostic_report.severity == error.expected_severity
                    and diagnostic_report.complexity == error.expected_complexity
                )
                else "⚠"
            )

            print(
                f"  [{i}/{len(all_errors)}] {status} {error.error_type} - {response_time:.2f}s"
            )

        except Exception as e:
            print(
                f"  [{i}/{len(all_errors)}] ✗ {error.error_type} - Error: {str(e)[:50]}"
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print(
                "Usage: python run_evaluation.py [num_constant_errors] [num_code_gen_errors]"
            )
